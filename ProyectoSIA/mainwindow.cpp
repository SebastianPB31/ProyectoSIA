...
void MainWindow::on_listaProducto_clicked() {
  ...
  // SQL query
  // imprimir a pantalla
  model->setQuery("SELECT * FROM REG_PROVISION", db);
...
void MainWindow::on_agregarNuevoProducto_clicked() {
  ...
  model->setQuery(query.arg(ui->codigoProd->text())
                      .arg(ui->tipoProd->currentData().toInt())
...
void MainWindow::on_codigoPistola_returnPressed() {
  ...
  if (verif == codigo) {
    ...
    ListaCompra.append(std::tuple<int, int, int, QString>(
        codigo.toInt(), cantidadI, precio, linea));
...
void MainWindow::on_cantidad_returnPressed() { ui->codigoPistola->setFocus(); }
